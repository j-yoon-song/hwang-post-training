...
def build_judge_messages(
    ...
    user = (
        ...
        "Check all criteria:\n"
        "1) Source quality gate: if source text is too noisy/non-linguistic (junk markup, link farm, "
        "navigation boilerplate, broken fragments, unreadable corruption), reject.\n"
        "2) Language: output must be in target language (not source language or mixed heavily).\n"
        "3) Fidelity: preserve meaning, facts, entities, numbers, dates, units, negation, and relations.\n"
        "4) Completeness: no major omission, truncation, or hallucinated additions.\n"
        "5) Format: translation text only (no 'Here is the translation', role tags, code fences, or explanations).\n"
        "6) Critical errors dominate: a single severe error means reject.\n\n"
        "Use one reason_code from this set:\n"
        "pass, wrong_language, severe_meaning_error, omission, addition_or_hallucination, "
        "format_violation, too_literal_or_ungrammatical, source_too_noisy, unsafe_or_corrupted.\n\n"
...
def build_round_trip_judge_messages(
    ...
    user = (
        ...
        "Critical checks:\n"
        "1) If source text itself is too noisy/non-linguistic to judge reliably, reject.\n"
        "2) Facts/entities/numbers/dates/units preserved.\n"
        "3) Negation, modality, and causal/temporal relations preserved.\n"
        "4) No major omission, addition, contradiction, or role reversal.\n"
        ...
        "entity_or_number_error, source_too_noisy, unclear_or_corrupted.\n\n"
